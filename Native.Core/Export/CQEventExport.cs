...
namespace Native.App.Export
{
	...
	{	
		...
		{	
			return "9,cx.rain.cq.coolrepeater";	
		}	
		...
		{	
			...
			Type appDataType = typeof (AppData);	
			// 注册一个 CQApi 实例	
			AppInfo appInfo = new AppInfo ("cx.rain.cq.coolrepeater", 1, 9, "Cool复读机", "1.1.0", 2, "秋雨落", "这个问题我没办法和你解释，因为我只是一只小小的复读机。", authCode);	
			appDataType.GetRuntimeProperty ("CQApi").GetSetMethod (true).Invoke (null, new object[] { new CQApi (appInfo) });	
			AppData.UnityContainer.RegisterInstance<CQApi> ("cx.rain.cq.coolrepeater", AppData.CQApi);	
			// 向容器注册一个 CQLog 实例	
			appDataType.GetRuntimeProperty ("CQLog").GetSetMethod (true).Invoke (null, new object[] { new CQLog (authCode) });	
			AppData.UnityContainer.RegisterInstance<CQLog> ("cx.rain.cq.coolrepeater", AppData.CQLog);	
		...
		{	
			...
			{	
				CQDiscussMessageEventArgs args = new CQDiscussMessageEventArgs (AppData.CQApi, AppData.CQLog, 2, 4, "讨论组消息处理", "_eventDiscussMsg", 30000, subType, msgId, fromNative, fromQQ, msg.ToString(CQApi.DefaultEncoding), false);	
